...
static inline uint16_t sheller_get_circular_buff_length(const sheller_t *desc)
{
    uint16_t value = 0;
    if (desc != NULL) {
...
uint8_t sheller_push(sheller_t *desc, uint8_t byte)
{
    ...
    if (desc != NULL) {
        if (desc->rx_buff_end != desc->rx_buff_begin || desc->rx_buff_empty_flag == 1) {
            ...
            //Overflow signal
            qDebug() << "[ ERROR ] Sheller circular buffer overflow. Called by sheller_push line" << __LINE__ << "from sheller.cpp";
        }
    }
